...
if [[ "$MODE" == "revert" ]]; then
  ...
  YAML="opentelemetry-demo-with-catalogerror.yaml"
  MARKER_VERSION="$VERSION"
...
# 4. Wait for rollout of product-catalog and flagd
echo "[INFO] Waiting for product-catalog deployment to be ready..."
kubectl rollout status deployment/product-catalog --timeout=120s
echo "[INFO] Waiting for flagd deployment to be ready..."
kubectl rollout status deployment/flagd --timeout=120s

# 5. Wait for all product-catalog/flagd pods to be Running and Ready
for dep in product-catalog flagd; do
